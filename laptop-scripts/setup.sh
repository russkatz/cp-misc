...
echo "Altering topic"
/opt/cp/cur/bin/kafka-configs --zookeeper node1:2181 --entity-type topics --entity-name topic01 --alter --add-config retention.ms=5000
...
curl -X DELETE http://node1:8083/connectors/replicate-topic01
curl -H "Content-Type: application/json" -d @"replicator.json" -X POST http://node1:8083/connectors
